...
int levelFetchFile(int argc, char const *argv[])
{

    fetchFile(filepath(levels[gameLevel].level) - 1);
    printTable();
    return 0;
}
...
void fetchRandomNumbers(int level)
{
    ...
    FILE* file;
    char buffer[MAX_LENGTH];  // Declares an array of characters to store a line read from the file. 
    ...
    if (fgets(buffer, MAX_LENGTH, file) != NULL)  //  Ito nagreread ng lines from textfile, tapos iniistore niya sa char buffer
    {											  // nichecheck if the reading is successful, if so will continue process inside brackets
...
void generateObstacles(int n)
{
    ...
    for (i = 0; i < (n - 1); i++)
    {
		...
		obstacles[i].xPos = rand() % (79 - strlen(obstacles[i].value) - 31) + 31;  // duplicate check, store in array
        obstacles[i].yPos = rand() % (BOARDHEIGHT - BOARDYPOS) + BOARDYPOS;    // duplicate check, store in array
        obstacles[i].isCorrectAnswer = false;
    }

    obstacles[n - 1].value = answer;
    obstacles[n - 1].xPos = rand() % (79 - strlen(obstacles[n - 1].value) - 31) + 31;
